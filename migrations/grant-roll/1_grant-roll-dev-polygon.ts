...
const handler = async function (deployer, network) {
	...
	const devPolygonInstance = await artifacts.require('DevPolygon').at(process.env.DEV_POLYGON!)
	...
	const hasBurnerRoll = await devPolygonInstance.hasRole(bunerRole, devBridgeAddress)
	console.log(`burner roll:${hasBurnerRoll}`)
	const hasMinterRoll = await devPolygonInstance.hasRole(minterRole, devBridgeAddress)
