	...
	describe('Lockup; depositToProperty', () => {
		describe('success', () => {
			it('get nft token.', async () => {
				...
				expect(position.amount.toNumber()).to.be.equal(100)
				expect(position.price.toNumber()).to.be.equal(0)
				expect(position.cumulativeReward.toNumber()).to.be.equal(0)
				expect(position.pendingReward.toNumber()).to.be.equal(0)
			})
			it('get 2 nft token.', async () => {
				...
				expect(position.property).to.be.equal(property.address)
				expect(position.amount.toNumber()).to.be.equal(200)
				expect(position.price.toString()).to.be.equal(
	...
	describe('Lockup; depositToPosition', () => {
		describe('success', () => {
			it('update nft.', async () => {
				...
				expect(beforePosition.amount.toNumber()).to.be.equal(100)
				expect(beforePosition.price.toNumber()).to.be.equal(0)
				expect(beforePosition.cumulativeReward.toNumber()).to.be.equal(0)
				expect(beforePosition.pendingReward.toNumber()).to.be.equal(0)
				...
				expect(afterPosition.amount.toNumber()).to.be.equal(200)
				expect(afterPosition.price.toString()).to.be.equal(
					toBigNumber('100000000000000000000000000000000000')
						.times(t2 - t1)
						.toFixed()
				)
				expect(afterPosition.cumulativeReward.toString()).to.be.equal(
					toBigNumber('10000000000000000000')
						.times(t2 - t1)
						.toFixed()
				)
				expect(afterPosition.pendingReward.toString()).to.be.equal(
					toBigNumber('10000000000000000000')
						.times(t2 - t1)
						.toFixed()
				)
	...
	describe('Lockup; withdrawByPosition', () => {
		describe('success', () => {
			it('update nft position.', async () => {
				...
				const afterPosition = await dev.sTokensManager.positions(tokenId)
				expect(afterPosition.property).to.be.equal(property.address)
				expect(afterPosition.amount.toNumber()).to.be.equal(0)
				expect(afterPosition.price.toString()).to.be.equal(
					toBigNumber('100000000000000000000000000000000000')
						.times(t2 - t1)
						.toFixed()
