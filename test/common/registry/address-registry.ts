...
contract('AddressRegistryTest', ([deployer, other, setAddress1, setAddress2]) => {
	describe('AddressRegistry; setRegistry/registries', () => {
		...
		describe('fail', () => {
			it('Value set by non-owner', async () => {
				const dev = await init()
				const result = await dev.addressRegistry.setRegistry('Allocator', setAddress1, {
								from: other,
				}).catch(
					errorCatch
				)
			...
			it('Value set by non-PolicyFactory', async () => {
				const dev = await init()
				const result = await dev.addressRegistry.setRegistry('Policy', setAddress1).catch(
					errorCatch
				)
				validateErrorMessage(result, 'this is illegal address')
			})
