...
contract STokensDescriptor {
	...
	function getTokenURI(
		...
		bool hasTokenUriImage = bytes(_tokeUriImage).length != 0;
		string memory imageMetaData = hasTokenUriImage
			? ""
			: "data:image/svg+xml;base64,";
		string memory image = hasTokenUriImage
			? string(abi.encodePacked(_tokeUriImage))
			: string(
				abi.encodePacked(
